function drawPolarAxis(ax, position, scale=1, tick_space=1, line_width=1, line_color=[0.1 0.1 0.1])
    ...
    for i = 1:length(r)
        X(:,:,i) = r(i) * scale * cos(angle);
        Y(:,:,i) = r(i) * scale * sin(angle);
        ...
        scale_label = sprintf('%.1f', r(i));
        if (i==length(r))
            scale_label = strcat(scale_label, ' N');
        end
        x_axis_offset = -0.01;
        text(position(1)+r(i)*scale, position(2)+x_axis_offset, scale_label, 'FontSize',12, 'HorizontalAlignment', 'center')
    end
    ...
    for i = 1:length(angle)
        ...
        A = [0 r(end) * scale * cos(angle(i))];
        B = [0 r(end) * scale * sin(angle(i))];
